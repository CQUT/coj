...
if (isset($_POST['id'])) {
	...
	if ($rows_cnt!=1){
		echo "<font color=red size=25>你不能参加该竞赛(作业)或竞赛(作业)已结束，所以不能提交!!</font>";
...
$statusURI=strstr($_SERVER['REQUEST_URI'],"submit",true)."http://coj.cqut.edu.cn/status.php";
if (isset($cid)) 
	$statusURI.="?cid=$cid";
...
$statusURI="http://coj.cqut.edu.cn/status.php?user_id=".$_SESSION['user_id'];
if (isset($cid))
	$statusURI.="&cid=$cid";
