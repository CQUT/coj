  ...
  function formatTimeLength($length)
{
	...
	if (isset($_GET['cid'])){
		...
		//	print $cid;
			...
			// check contest valid
			$sql="SELECT * FROM `contest` WHERE `contest_id`='$cid' and 'private'=2";
  ...
  $sql="select *  from contest left join (select * from privilege where rightstr like 'm%') p on concat('m',contest_id)=rightstr where contest.private='2' and contest.defunct='N' and contest.title like '%$keyword%'  order by contest_id desc limit 1000;";
			$result=mysql_query($sql);
