...
<?php require_once("../include/simple_html_dom.php");
  ...
  if (strpos($url, "http") === false){
	echo "Please Input like http://www.codeup.cn/problem.php?id=3933";
	exit(1);
  }
  ...
  $element=$html->find('b',0);
  $tlimit=$element->plaintext; 
  $element=$html->find('b',1);
  $mlimit=$element->plaintext;
  $tlimit=1;
  $mlimit=128;
  
  $element=$html->find('div[class=content-wrapper well]',0);
  $descriptionHTML=$element->outertext;
  $element=$html->find('div[class=content-wrapper well]',1);
  $inputHTML=$element->outertext;
  $element=$html->find('div[class=content-wrapper well]',2);
  $outputHTML=$element->outertext;
  $element=$html->find('div[class=content-wrapper well]',3);
  $sample_input=$element->find('span[class=sampledata]',0)->innertext;
  $element=$html->find('div[class=content-wrapper well]',4);
  $sample_output=$element->find('span[class=sampledata]',0)->innertext;
