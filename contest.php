...
//  print $cid;
// check contest valid
    $sql="SELECT * FROM `contest` WHERE `contest_id`='$cid' and ('private'=0 or 'private'=1)";
...
// echo "$keyword";
    ...
    $sql="select *  from contest left join (select * from privilege where rightstr like 'm%') p on concat('m',contest_id)=rightstr where (contest.private='0' or contest.private='1')and contest.defunct='N' and contest.title like '%$keyword%'  order by contest_id desc limit 1000;";
    $result=mysql_query($sql);
    $view_contest=Array();
    $i=0;
    while ($row=mysql_fetch_object($result)){
